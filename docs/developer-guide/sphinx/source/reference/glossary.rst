...
Common terms and concepts used in Marcus documentation.

.. glossary::
   :sorted:

   Agent
      An AI worker that connects to Marcus to complete tasks. Agents have specific skills and roles.
   ...
   Blocker
      An issue preventing a worker from completing a task. Marcus provides AI-powered resolution suggestions.
   ...
   Claude
      Anthropic's AI assistant, commonly used as a worker agent with Marcus.

   Column
      A vertical section of a kanban board representing a task state (e.g., Todo, In Progress, Done).

   Conversation Log
      Detailed record of communications between workers, Marcus, and the kanban board.

   Docker
      Container platform used to package and run Marcus and its dependencies.

   Docker Compose
      Tool for defining and running multi-container Docker applications, used for Marcus deployment.
   ...
   Linear
      Cloud-based project management tool that Marcus can integrate with.

   MCP
      Model Context Protocol - Standard for AI model communication used by Marcus.

   MCP Client
      Component that connects to an MCP server to use its tools (Marcus connects to kanban-mcp).

   MCP Server
      Component that exposes tools via MCP (Marcus acts as server for worker agents).

   MCP Tool
      Specific function exposed via MCP that agents can call (e.g., register_agent, request_next_task).

   Planka
      Open-source kanban board that runs locally, used as default task board for Marcus.
   ...
   Provider
      Task management system Marcus integrates with (GitHub, Linear, or Planka).

   Registration
      Process where a worker agent declares its identity and capabilities to Marcus.

   Skill
      Technical capability of a worker (e.g., "python", "react", "docker") used for task matching.

   Sphinx
      Documentation generator used to create Marcus's documentation from RST and Markdown files.
   ...
   Visualization
      Real-time graphical display of Marcus's operation, showing data flow and decisions.
