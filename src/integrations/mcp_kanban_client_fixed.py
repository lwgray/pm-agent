...
class MCPKanbanClient:
    """Client for interacting with MCP Kanban server - Fixed for MCP 1.1.0"""
    ...
    async def connect(self):
        """Connect to the MCP Kanban server - Fixed for MCP 1.1.0"""
        ...
        async with stdio_client(server_params) as (read, write):
            ...
            # Keep the connection alive by not exiting the context
            await self._keep_alive()
    
    async def _keep_alive(self):
        """Keep the connection alive"""
        # This is a workaround - we need to keep the stdio streams open
        # In a real implementation, you'd want a proper connection manager
        while self.session:
