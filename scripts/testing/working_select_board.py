#!/usr/bin/env python3
"""
Working version of select_task_master_board.py
"""
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            if len(boards) == 1:
                ...
                print(f"\n✅ Auto-selected the only board: {selected_board['name']}")
            else:
                choice = input("\nSelect board number (or press Enter for first board): ").strip()
                if not choice:
                    selected_board = boards[0]
                else:
                    try:
            ...
            print(f"  Completed cards: {summary['completedCardCount']}")
            print(f"  Total tasks: {summary['totalTaskCount']}")
            print(f"  Completed tasks: {summary['completedTaskCount']}")
            
            if summary['lists']:
                print("\nLists:")
                for lst in summary['lists']:
                    print(f"  - {lst['name']} ({lst['cardCount']} cards)")
